...
for p_d_w, q_d_w, p_l_c, q_l_c, r_g_g0 in zip(p_D_W_mocap, q_D_W_mocap, p_L_C, q_L_C, R_G_G0):
    ...
    q_d_w = np.array(q_d_w)
    r_d_w = R.from_quat(q_d_w)
    
    # 2. Drone -> Gimbal base
    p_G0_D = np.array([0,0,-0.107]) # 10.7 cm from drone to gimbal
    ...
    q_G_G0 = R.from_euler('XY', [r_g_g0[0]+2.3, r_g_g0[1]-1.3], degrees=True).as_quat()  # [x,y,z,w]
...
for ax, (gt, est, label, error) in zip(axes[:3], translation_components):
    ...
    ax.set_ylabel(label)
    ax.set_title(f"{label} Translation (RMSE={error:.4f}m)")
    ax.legend()
    ax.grid(True)
