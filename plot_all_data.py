...
with open(csv_path, newline='') as f:
    reader = csv.reader(f)
    for row in reader:
        p_C_W.append([float(row[0]), float(row[1]), float(row[2])])
        q_C_W.append([float(row[3]), float(row[4]), float(row[5]), float(row[6])])

        p_L_C.append([float(row[7]), float(row[8]), float(row[9])])
        q_L_C.append([float(row[10]), float(row[11]), float(row[12]), float(row[13])])

        p_L_W.append([float(row[14]), float(row[15]), float(row[16])])
        q_L_W.append([float(row[17]), float(row[18]), float(row[19]), float(row[20])])

# ---- ESTIMATE POSE ----
p_L_W_est = []
R_L_W_est = []

for p_c_w, q_c_w, p_l_c, q_l_c in zip(p_C_W, q_C_W, p_L_C, q_L_C):

    R_C_W = R.from_quat(q_c_w)
    R_L_C = R.from_quat(q_l_c)

    # position
    p_L_W_est.append(R_C_W.apply(p_l_c) + p_c_w)

    # orientation composition (Extend with 180 degree rotation around x if needed to align frames)
    R_L_W_est.append(R_C_W * R_L_C * R.from_euler('x', 180, degrees=True)) # Uncomment if load estimation frame is rotated 180 deg around x from camera frame

    #print(f"quarternion load in camera frame: {R.from_quat(q_l_c).as_euler('xyz', degrees=True)}")
    #print(f"quarternion load in world frame: {R.from_quat(q_L_W[0]).as_euler('xyz', degrees=True)}")
...
# ---- TRANSLATION RMSE ----
rmse_x = rmse([p[0] for p in p_L_W], [p[0] for p in p_L_W_est])
rmse_y = rmse([p[1] for p in p_L_W], [p[1] for p in p_L_W_est])
rmse_z = rmse([p[2] for p in p_L_W], [p[2] for p in p_L_W_est])
...
R_L_W_gt = [R.from_quat(q) for q in q_L_W]
...
# ---- SAMPLE INDICES ----
samples = list(range(1, len(p_L_W) + 1))

# ---- PLOTTING ----
fig, axes = plt.subplots(6, 1, figsize=(12, 16), sharex=True)

translation_components = [
    ([p[0] for p in p_L_W], [p[0] for p in p_L_W_est], "X", rmse_x),
    ([p[1] for p in p_L_W], [p[1] for p in p_L_W_est], "Y", rmse_y),
    ([p[2] for p in p_L_W], [p[2] for p in p_L_W_est], "Z", rmse_z),
]
