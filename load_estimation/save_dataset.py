...
class SaveDatasetNode(Node):
    def __init__(self):
        ...
        # ========== Subscribers ==========
        self.subscribe_load_pose_ = self.create_subscription(Vector3, '/payload/pose', self.load_pose_callback, 2)
    ...
    def load_pose_callback(self, msg):
        if self.camera_pose is not None and self.mocap_load_pose is not None:
            mocap_camera_payload = self.mocap_load_pose.pose.position.x - self.camera_pose.pose.position.x, \
                                   self.mocap_load_pose.pose.position.y - self.camera_pose.pose.position.y, \
                                   self.mocap_load_pose.pose.position.z - self.camera_pose.pose.position.z
            


            with open('dataset.csv', 'a') as f:
                f.write(f"{msg.x},{msg.y},{msg.z},{mocap_camera_payload[0]},{mocap_camera_payload[1]},{mocap_camera_payload[2]}\n")
    ...
    def load_pose_callback_old(self, msg):
        if self.camera_pose is not None and self.mocap_load_pose is not None:
            ...
            p_L_C = np.array([msg.x, msg.y, msg.z])
            ...
            q_C_W = R.from_quat([-0.7071, 0, 0, 0.7071]) * R.from_quat(q_C_W) # 90 deg around x
